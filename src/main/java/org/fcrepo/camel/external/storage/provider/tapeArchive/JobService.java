...
public class JobService implements Processor {
    ...
    public void process(final Exchange exchange) throws Exception {
        ...
        System.out.println("Inside JobService processing pending tapeArchive jobs...");
        List<Job> tapeArchive_jobs = exchange.getIn().getBody(List.class);
        if (tapeArchive_jobs != null && !tapeArchive_jobs.isEmpty()){
            for (Job job : tapeArchive_jobs) {
                // TODO Have to conditionally handle fixity
                String result = template.requestBodyAndHeader("direct:tapeArchiveGetCache", "mybody", "external_uri", 
